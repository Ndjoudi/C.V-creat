...
Réponds avec ce JSON exact :
{
  ...
  "score": 1-5,
  "points_forts": ["max 3 forces du candidat pour CE poste (phrase courte, cite le CV)"],
  "match": [
    {"exigence": "texte exact de l'exigence", "ligne_cv": "ligne exacte du CV qui couvre ça, vide si absent", "niveau": "ok|partial|gap"}
  ],
  "lacunes": [
    {"lacune": "ce qui manque", "bloqueur": true/false, "mitigation": "comment compenser en 1 phrase courte"}
  ],
...
- "lacunes" : uniquement les vraies absences du CV (pas ce qui est partiellement couvert)
...
- "recommandations_cv" : actions concrètes ("Ajouter X dans le résumé", "Mentionner Y dans l'expérience Z")`;
  ...
  try {
    ...
    const raw    = await callFn(prompt, { maxTokens: 1200, temperature: 0.1 });
...
function _renderCareerOpsResult(d, candId, provider, el) {
  ...
  el.innerHTML = metaHtml + fortsHtml + matchHtml + lacunesHtml + recoHtml;
}
