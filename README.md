...
| 2026-09-16 | **Import du CV par dépôt d'un PDF** (le champ « colle ton texte » est supprimé) | Le texte est extrait avec le lecteur PDF déjà embarqué (`_chargePdfJs` / `_texteDuDocument` de `js/lettre-reco.js`), puis analysé par `callAIAuto` (Gemini puis Groq). Un CV scanné est refusé avec un message clair : il n'y a pas d'OCR |
| 2026-09-16 | **Le bouton favori lit aussi les cadres internes (iframes)** | En vue « liste à gauche, détail à droite », Indeed affiche l'offre dans une iframe : le favori ne trouvait ni titre ni description. Il parcourt maintenant la page et les iframes accessibles, avec un secours sur tout bloc dont l'id/la classe contient « description » ou « job-details ». Rappel : un favori déjà dans la barre garde l'ancien code, il faut le reglisser |
| 2026-09-16 | **Poste et entreprise : emplacements de secours** | En vue liste, Indeed ne met le titre et l'entreprise ni dans l'iframe ni aux emplacements habituels : le favori lit aussi la carte sélectionnée (`.vjs-highlight`), les `data-testid` « simpler-… », et en dernier recours le titre de l'onglet « Poste - Entreprise - Indeed.com ». Sans eux, le site refusait d'enregistrer (« Poste ou entreprise introuvable ») ; ce message indique maintenant ce qui a été reçu |
| 2026-09-16 | **Le poste enregistré prime sur celui de l'analyse IA** | L'IA reprenait parfois l'accroche de l'annonce comme intitulé (« CHASSEUR D'OR » au lieu de « Responsable d'agence »). CV, PDF et fiche de l'annonce utilisent `c.poste || a.poste` (et `c.company || a.entreprise`), comme le tableau de bord |
...
| 2026-09-24 | **Sauvegarde automatique hebdomadaire** | Perte totale des données (localStorage vidé). Le site télécharge maintenant un fichier complet tous les 7 jours, restaurable en un clic. Au passage : `P` est complété avec `DEF_PROFILE` au démarrage, car un profil restauré incomplet faisait planter le chargement (page blanche sur `P.experiences.forEach`) |
| 2026-09-24 | **HelloWork ajouté au bouton favori** | Les pages HelloWork contiennent une fiche structurée JSON-LD `JobPosting` : le favori la lit (titre, entreprise, ville + code postal, salaire, description) plutôt que des sélecteurs de mise en page. Import, pastille et filtre « HelloWork » ajoutés au tableau de bord. Lecture impossible depuis le site (requête bloquée) : message explicite renvoyant vers le favori |
| 2026-09-24 | **Réalisations réordonnables dans le profil** | Flèches ▲▼ sur chaque réalisation (`deplaceBullet` dans `js/profile.js`), désactivées en haut et en bas de liste. L'ordre du profil est celui du CV : le CV et l'annonce ouverte sont redessinés aussitôt |
